...
class Model {
    ...
    Model(Model model) {
        ...
        _score = model.score(); _streakLength = model._streakLength;
        _hand = model._hand;
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            ...
            for (int j = 0; j < piece.width(); j++) {
                ...
                if (piece.get(i, j) && _cells[i + row][j + col]) {
                    return false;
                }
            }
        }
    ...
    void place(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            for (int j = 0; j < piece.width(); j++) {
                if (piece.get(i, j)) {
                    _cells[i + row][j + col] = piece.get(i, j);
                    _score++;
                }
            }
        }
    ...
    void clearFilledLines() {
        ...
        for (int i = 0; i < counts.length; i++) {
            for (int j = 0; j < counts[0].length; j++) {
                if (i == 0 && counts[i][j] == _width) {
                    nrows++;
                    clearRow(j);

                }
                if (i == 1 && counts[i][j] == _height) {
                    ncols++;
                    clearCol(j);
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        ...
        base = ((nrows * _width) + (ncols * _height)) - (nrows * ncols);
        bonus = ((nrows * _width) + (ncols * _height)) * (_streakLength);
